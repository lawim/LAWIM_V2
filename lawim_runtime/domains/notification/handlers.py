...
class NotificationHandler(DomainRuntimeHandler):
    handler_name: str = "notification_handler"
    ...
    def __init__(self, runtime: NotificationRuntime | None = None) -> None:
        self._runtime = runtime or NotificationRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by notification handler"]
