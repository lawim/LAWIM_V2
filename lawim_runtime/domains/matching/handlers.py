...
class MatchingHandler(DomainRuntimeHandler):
    handler_name: str = "matching_handler"
    ...
    def __init__(self, runtime: MatchingRuntime | None = None) -> None:
        self._runtime = runtime or MatchingRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by matching handler"]
