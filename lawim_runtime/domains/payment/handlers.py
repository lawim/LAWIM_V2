...
class PaymentHandler(DomainRuntimeHandler):
    handler_name: str = "payment_handler"
    ...
    def __init__(self, runtime: PaymentRuntime | None = None) -> None:
        self._runtime = runtime or PaymentRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by payment handler"]
