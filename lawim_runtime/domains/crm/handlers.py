...
class CRMHandler(DomainRuntimeHandler):
    handler_name: str = "crm_handler"
    ...
    def __init__(self, runtime: CRMRuntime | None = None) -> None:
        self._runtime = runtime or CRMRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by crm handler"]
