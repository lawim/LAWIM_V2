...
class TransactionHandler(DomainRuntimeHandler):
    handler_name: str = "transaction_handler"
    ...
    def __init__(self, runtime: TransactionRuntime | None = None) -> None:
        self._runtime = runtime or TransactionRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by transaction handler"]
