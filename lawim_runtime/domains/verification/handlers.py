...
class VerificationHandler(DomainRuntimeHandler):
    handler_name: str = "verification_handler"
    ...
    def __init__(self, runtime: VerificationRuntime | None = None) -> None:
        self._runtime = runtime or VerificationRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by verification handler"]
