...
class DocumentHandler(DomainRuntimeHandler):
    handler_name: str = "document_handler"
    ...
    def __init__(self, runtime: DocumentRuntime | None = None) -> None:
        self._runtime = runtime or DocumentRuntime()
    ...
    def validate(self, context: ActionExecutionContext) -> list[str]:
        ...
        if action_code not in self.supported_actions:
            return [f"action_code '{action_code}' not supported by document handler"]
