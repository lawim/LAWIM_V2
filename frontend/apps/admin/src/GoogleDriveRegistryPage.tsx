...
export function GoogleDriveRegistryPage() {
  ...
  return (
    <PageShell
      ...
      description="Logical Drive records, credential references, OAuth state, quota, routing, and the secure onboarding perimeter."
    >
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card title="Drive fleet" description="Capacity, health, OAuth state, credential binding, and incident trace for each Drive.">
            ...
            {snapshot.connectors.map((drive) => (
              <div key={drive.driveId} className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4">
                ...
                <div className="mt-4 space-y-2 text-sm text-slate-300">
                  <div className="rounded-xl border border-slate-800 px-3 py-2">Credential ID: {drive.credentialId}</div>
                  ...
                  <div className="rounded-xl border border-slate-800 px-3 py-2">OAuth: {drive.oauthStatus} · Credential: {drive.credentialStatus} · API: {drive.apiVersion}</div>
                </div>
                ...
                <div className="mt-4 space-y-2 text-xs text-slate-500">
                  ...
                  <div>Last connection test: {drive.lastConnectionTest}</div>
                  <div>Last upload test: {drive.lastUploadTest}</div>
                  <div>Last download test: {drive.lastDownloadTest}</div>
                  <div>Last healthcheck: {drive.lastHealthcheck}</div>
        ...
        <Card title="Operational rules" description="The registry is ready for secure onboarding of the real Google accounts.">
          <div className="mt-4 space-y-3 text-sm text-slate-300">
            ...
            <div className="rounded-xl border border-slate-800 px-3 py-2">The Credential Vault keeps protected material encrypted outside Git, logs, docs, and the frontend.</div>
            ...
            <div className="rounded-xl border border-slate-800 px-3 py-2">Credential Vault: {snapshot.credentialVault.summary.recordCount} records · {snapshot.credentialVault.summary.maskedRecords} masked · {snapshot.credentialVault.summary.protectionLayers} protection layers.</div>
          </div>
        </Card>
      </div>
        ...
        <Card title="Monitoring" description="Quota, latency, throughput, API state, and alert posture.">
          <div className="mt-4 space-y-3 text-sm text-slate-300">
            ...
            <div className="flex items-center justify-between rounded-xl border border-slate-800 px-3 py-2">
              ...
              <Badge variant={badgeVariantForStatus(snapshot.credentialVault.status)}>{snapshot.credentialVault.summary.recordCount}</Badge>
            </div>
            <div className="flex items-center justify-between rounded-xl border border-slate-800 px-3 py-2">
              <span>Security controls</span>
              <Badge variant="info">{snapshot.security.controls.length}</Badge>
            </div>
          </div>
        </Card>
