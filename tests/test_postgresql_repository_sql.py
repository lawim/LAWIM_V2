...
from lawim_v2.postgresql_repository import _PgConnection, _adapt_sql, _should_return_postgresql_id
...
class PostgreSQLRepositorySqlTest(unittest.TestCase):
    ...
    def test_adapt_sql_rewrites_sqlite_specific_functions_for_postgresql(self) -> None:
        sql = "SELECT IFNULL(name, 'unknown'), strftime('%Y-%m-%d', created_at), datetime('now', '-1 day'), julianday(created_at), id INTEGER PRIMARY KEY AUTOINCREMENT"

        adapted = _adapt_sql(sql)

        self.assertIn("COALESCE(name, 'unknown')", adapted)
        self.assertIn("to_char(created_at, '%Y-%m-%d')", adapted)
        self.assertIn("CURRENT_TIMESTAMP - INTERVAL '1 day'", adapted)
        self.assertIn("EXTRACT(EPOCH FROM created_at)", adapted)
        self.assertIn("GENERATED BY DEFAULT AS IDENTITY", adapted)
