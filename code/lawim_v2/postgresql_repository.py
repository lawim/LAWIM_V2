...
def _adapt_sql(sql: str) -> str:
    ...
    adapted = _rewrite_sqlite_specific_sql(adapted)
...
def _rewrite_sqlite_specific_sql(sql: str) -> str:
    adapted = re.sub(
        r"\bIFNULL\s*\(\s*([^,]+?)\s*,\s*([^\)]+?)\s*\)",
        lambda match: f"COALESCE({match.group(1).strip()}, {match.group(2).strip()})",
        sql,
        flags=re.IGNORECASE,
    )
    adapted = re.sub(
        r"\bstrftime\s*\(\s*(['\"])(.*?)\1\s*,\s*([^\)]+?)\s*\)",
        lambda match: f"to_char({match.group(3).strip()}, {match.group(1)}{match.group(2)}{match.group(1)})",
        adapted,
        flags=re.IGNORECASE,
    )
    adapted = re.sub(
        r"\bdatetime\s*\(\s*'now'\s*,\s*'([^']+)'\s*\)",
        lambda match: f"CURRENT_TIMESTAMP - INTERVAL '{match.group(1).lstrip('-')}'",
        adapted,
        flags=re.IGNORECASE,
    )
    adapted = re.sub(
    ...
    adapted = re.sub(
        r"\bjulianday\s*\(\s*([^\)]+?)\s*\)",
        lambda match: f"EXTRACT(EPOCH FROM {match.group(1).strip()})",
        adapted,
        flags=re.IGNORECASE,
    )
    adapted = re.sub(
