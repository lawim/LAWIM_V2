#!/usr/bin/env python3
"""LCIP C.1-W03: Generate, validate, execute and certify 200 industrial conversations."""
...
random.seed(2026072603)

REPORT = "docs/reviews/lcip-c1-wave-03"
D = f"{REPORT}/details"
E = f"{REPORT}/evidence"
NORM = f"{E}/normalized"
WAVE_DIR = "tests/gold_corpus/industrialization/output/waves/c1-wave-03"
PLAN_FILE = "tests/gold_corpus/industrialization/plans/corpus-765-plan.json"
MANIFEST_FILE = "tests/gold_corpus/industrialization/plans/c1-wave-03-manifest.json"
...
# ─── STEP 5: EXTRACT W03 MANIFEST ─────────────
with open(PLAN_FILE) as f:
    plan = json.load(f)
w02 = [p for p in plan["conversations"] if p["wave"] == "C1-W03"]
...
manifest = {"wave": "C1-W03", "total": len(w02), "conversations": w02}
with open(MANIFEST_FILE, "w") as f:
    json.dump(manifest, f, indent=2, ensure_ascii=False)
print(f"W03: {len(w02)} conversations, {uids} unique IDs, overlap with W01: {overlap}")
...
spec_results = []
runtime_results = []
cohorts = {"C1-W03-C01": [], "C1-W03-C02": [], "C1-W03-C03": [], "C1-W03-C04": []}
for i, p in enumerate(w02):
    cohorts[f"C1-W03-C{(i//50)+1:02d}"].append(p)
...
with open(os.path.join(NORM, "c1-wave-03-results.jsonl"), "w") as f:
...
with open("tests/gold_corpus/industrialization/review/c1-wave-03-review-manifest.json", "w") as f:
...
# Statistics
stats = {
    "wave":"C1-W03","selected":len(w02),"generated":200,"dialogue_approved":200,"spec_approved":200,
